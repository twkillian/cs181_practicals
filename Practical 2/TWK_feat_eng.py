...
def create_data_matrix(start_index, end_index, direc="train", training=True):
    X = None
    classes = []
    ids = [] 
    i = -1
...
def call_feats(tree, good_calls):
    ...
    call_feat_array = np.append(call_feat_array,len(call_counter.keys()))
...
def main():
    ...
    X_train, t_train, train_ids = create_data_matrix(0, num_of_train_files, TRAIN_DIR, training=True)
    X_test, t_test, test_ids = create_data_matrix(0, num_of_test_files, TEST_DIR, training=False)
